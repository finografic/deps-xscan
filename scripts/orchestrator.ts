...
import { generateReport, OutputFormat } from "./report";
...
async function main(): Promise<void> {
  ...
  log(
    `  Found ${lockResult.deps.length} deps (${lockResult.format} format)`,
    args.verbose
  );
  ...
  let nodeVersion = lockResult.nodeVersion;
  if (!nodeVersion) {
    try {
      const { execSync } = require("child_process");
      const runtimeVersion = execSync("node --version", { encoding: "utf-8" })
        .trim()
        .replace(/^v/, "");
  ...
  try {
    ...
    const vulnCount = osvResults.reduce(
