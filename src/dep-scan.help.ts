...
export function printHelp(): void {
  ...
  lines.push('');
  lines.push(`${pc.bold('deps-xscan')} - TODO: describe your CLI tool`);
  lines.push('');

  lines.push(pc.bold('USAGE'));
  lines.push(`  ${pc.cyanBright('deps-xscan')} ${pc.dim(pc.cyan('<command>'))} [options]`);
  lines.push('');

  lines.push(pc.bold('COMMANDS'));
  const commands = [{ name: 'TODO', desc: 'Add your commands here' }];
  ...
  const examples = [{ cmd: 'deps-xscan TODO', comment: 'TODO: describe this example' }];
  ...
  lines.push(
    `  ${pc.cyanBright('deps-xscan')} ${pc.dim(pc.cyan('<command>'))} --help       ${pc.dim('# Show detailed help for a command')}`,
  );
  lines.push('');
